...
const chartConfig = {
  quality: { label: 'Quality', color: '#2f6fed' },
  improvement: { label: 'Improvement', color: '#8b5cf6' },
  dislocation: { label: 'Dislocation', color: '#0f9f75' },
  event: { label: 'Event', color: '#d97706' },
    ...
    () =>
      filteredCandidates.map((candidate) => {
        ...
        return {
          ...
          lens,
          x: result.evidence_ratio_pct,
          y: result.coverage_pct,
        };
      }),
    [filteredCandidates, selectedLens],
  );
        ...
        <div className="mx-auto grid h-full max-w-[1800px] grid-cols-1 lg:grid-cols-[220px_minmax(0,1fr)]">
          ...
          <main className="flex min-w-0 flex-col overflow-hidden">
            ...
            <div className="flex min-h-0 flex-1 flex-col gap-3 px-4 pb-4 pt-4 sm:px-5 xl:px-6">
              ...
              <div className="grid min-h-0 flex-1 gap-3 xl:grid-cols-[250px_minmax(440px,1fr)_340px]">
                <RadarSidebar
  ...
  return (
    ...
    >
      ...
      <nav className="space-y-1" aria-label="주요 메뉴">
        <NavItem icon={LayoutGrid} label="Overview" disabled />
        <NavItem
          icon={Radar}
          label="Radar"
          active
        ...
        <NavItem icon={BookOpen} label="Explore" trailing="준비" disabled />
      </nav>
      <p className="mb-2 mt-7 px-3 text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">
        System
      </p>
      <nav className="space-y-1" aria-label="시스템 메뉴">
        <NavItem icon={Database} label="Data health" disabled />
  ...
  const metrics = [
    {
      ...
      sub: `분석가능 재무 ${financialCount.toLocaleString('ko-KR')}개`,
    },
    {
      label: '공통 게이트 통과',
    ...
    {
      label: '규칙 버전',
      value: radarRun.rule_version.replace('2026-09-02.', ''),
...
function RadarMap({
  ...
  const displayLens = selectedCandidate
    ? lensForDisplay(selectedCandidate, selectedLens)
    : null;
  const result =
    selectedCandidate && displayLens
      ? selectedCandidate.lenses[displayLens]
      : null;
  return (
    <section className="flex min-h-[480px] min-w-0 flex-col overflow-hidden rounded-[20px] border border-border bg-card shadow-[0_12px_40px_rgba(15,23,42,0.035)] xl:min-h-0">
      <div className="flex shrink-0 items-center justify-between border-b border-border px-5 py-3.5">
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-sm font-semibold">Radar map</h2>
            <Badge
              variant="secondary"
          ...
          <p className="mt-1 text-[11px] text-muted-foreground">
            충족 근거 비율 × 데이터 커버리지 · 투자 점수가 아닙니다
          </p>
        </div>
        <div className="hidden items-center gap-3 text-[10px] text-muted-foreground 2xl:flex">
          {lenses.map((lens) => (
            <span key={lens.key} className="flex items-center gap-1.5">
              <span
      ...
      {chartPoints.length ? (
        <div className="min-h-[300px] flex-1 p-4">
          ...
          >
            <ScatterChart
              margin={{ top: 22, right: 26, bottom: 14, left: -12 }}
            >
              ...
              />
              <ReferenceLine x={50} stroke="#cbd5e1" strokeDasharray="4 5" />
              <ReferenceLine y={70} stroke="#cbd5e1" strokeDasharray="4 5" />
              <ChartTooltip
                    ...
                    formatter={(_value, _name, item) => (
                      <div className="min-w-36">
                        ...
                        <p className="mt-1 text-muted-foreground">
                          {item.payload.market} ·{' '}
                          {lensLabel[item.payload.lens as Lens]}
                        </p>
                      </div>
                    )}
                  />
              ...
              {chartPoints.map((point) => (
                <Scatter
                  ...
                  data={[point]}
                  cursor="pointer"
                  fill={lensColor[point.lens]}
                  fillOpacity={point.code === selectedCode ? 1 : 0.72}
                  stroke={point.code === selectedCode ? '#0f172a' : '#ffffff'}
                  strokeWidth={point.code === selectedCode ? 3 : 1.5}
                  onClick={() => onCandidateChange(point.code)}
                >
      ...
      {result ? (
        <div className="grid shrink-0 gap-px border-t border-border bg-border md:grid-cols-3">
          {result.evidence.slice(0, 3).map((item, index) => (
            <div key={item.key} className="bg-white px-4 py-3">
              <div className="flex items-center gap-2">
                <span className="grid size-5 place-items-center rounded-full bg-blue-50 text-[9px] font-bold text-primary">
                  {index + 1}
                </span>
...
function EvidencePanel({
  ...
  if (!candidate) {
    return (
      <aside className="flex min-h-[480px] items-center justify-center rounded-[20px] border border-primary/15 bg-[linear-gradient(180deg,#f7f9ff_0%,#ffffff_34%)] p-6 text-center">
        <div>
  ...
  return (
    <aside className="flex min-h-[480px] flex-col overflow-hidden rounded-[20px] border border-primary/15 bg-[linear-gradient(180deg,#f7f9ff_0%,#ffffff_34%)] shadow-[0_16px_50px_rgba(35,79,194,0.09)] xl:min-h-0">
      <div className="shrink-0 border-b border-primary/10 px-4 py-3.5">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-xs font-semibold">Evidence Copilot</p>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-4">
        ...
        <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-3.5">
          ...
          <div className="mt-3 flex items-center gap-3 text-[9px] text-muted-foreground">
            <span>충족 근거 {result.evidence_ratio_pct}%</span>
            <span>데이터 {result.coverage_pct}%</span>
            <span>{result.band === 'strong' ? '근거 다수' : '검토 필요'}</span>
          </div>
        </div>
      ...
      <div className="shrink-0 border-t border-primary/10 bg-white/80 p-3.5">
        ...
        >
          후보 기본 자료 살펴보기 <ExternalLink className="size-3" />
        </Link>
        <div className="mt-2">
...
function NavItem({
  ...
  const className = `flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-[13px] font-medium transition ${active ? 'bg-primary/[0.08] text-primary' : disabled ? 'cursor-not-allowed text-slate-400' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-950'}`;
  return href ? (
