...
export async function POST(request:Request){
 ...
 try{return Response.json({ok:true,job:await startDashboardJob('markets')},{status:202,headers:{'Cache-Control':'no-store'}});}catch{return Response.json({error:'수집을 시작하지 못했습니다.'},{status:503});}
}
